...
<template>
  ...
  <section v-if="userStore.selectedClass"
    ...
    <div class="text-sm text-gray-600 dark:text-gray-300 text-center">
      Level {{ userStore.selectedClass.baseStats.level }} /
      {{ userStore.selectedClass.baseStats.maxLevel }}<br />
      XP: {{ userStore.selectedClass.baseStats.xp }} /
      {{ userStore.selectedClass.baseStats.xpToNextLevel }}<br />
    ...
    <div class="grid grid-cols-2 gap-4 text-gray-700 dark:text-gray-200 text-sm">
      <div>
        <strong>HP:</strong>
        {{ userStore.selectedClass.baseStats.health }} /
        {{ userStore.selectedClass.baseStats.maxHealth }} <br />
        <strong>Regen:</strong>
        {{ userStore.selectedClass.baseStats.healthRegen }} /
    ...
    <div class="border-t border-gray-300 dark:border-gray-700 pt-4">
      ...
      <ul class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-gray-700 dark:text-gray-200">
        <li v-for="(value, name) in userStore.selectedClass.baseStats.attributes" :key="name">
    ...
    <section class="border-t border-gray-300 dark:border-gray-700 pt-4">
      <div v-for="skill in userStore.selectedClass.baseStats.skills?.filter(s => s.enabled)" :key="skill.name">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">
          {{ skill.name }}
        </h2>

        <progress class="progress w-56" :value="skill.progress" max="100" />
